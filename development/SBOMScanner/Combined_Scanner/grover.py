...
def prioritize_with_grover(confirmed_vuln_pkgs):
    """Prioritize packages deterministically with Grover flavor."""
    if not confirmed_vuln_pkgs:
        return []
    mask = [1] * len(confirmed_vuln_pkgs)
    return grover_simulate_order(confirmed_vuln_pkgs, mask, seed=12345)
...
def main():
    ...
    print("[i] Verifying packages against Ubuntu/Debian/NVD (version-aware)...")
    ...
    print(f"[i] Confirmed vulnerable packages: {len(confirmed)}")
